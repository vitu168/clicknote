        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={cn('group flex items-end gap-2', isMe ? 'flex-row-reverse' : 'flex-row')}>
              {/* Avatar */}
              {!isMe && (
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-accent-100 dark:bg-accent-950/50 text-[10px] font-bold text-accent-700 dark:text-accent-400">
              ...
              <div className={cn('relative max-w-xs lg:max-w-md', isMe ? 'items-end' : 'items-start')}>
                ...
                {msg.reply_to_text && (
                  <div className={cn(
                    ...
                    isMe ? 'border-accent-300 dark:border-accent-900/70 bg-accent-50 dark:bg-accent-950/30 text-right' : 'border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50',
                  )}>
                ...
                <div className={cn('mt-0.5 flex items-center gap-1.5', isMe ? 'justify-end' : 'justify-start')}>
                  ...
                  <div className="flex opacity-0 group-hover:opacity-100 transition-opacity gap-0.5">
                    <button
                      ...
                      title="Add reaction"
                      className="rounded p-0.5 text-slate-400 dark:text-slate-500 hover:text-accent-500 dark:hover:text-accent-400 transition-colors"
                    >
      ...
      <div className="border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-3">
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            title="Send message"
            className={cn(
              'flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-accent-600 text-white shadow-sm transition-colors',
              'hover:bg-accent-700 disabled:opacity-40 disabled:cursor-not-allowed',
            )}
          >
