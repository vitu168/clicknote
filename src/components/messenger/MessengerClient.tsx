...
const AVATAR_COLORS = [
  'bg-accent-500', 'bg-accent-500', 'bg-sky-500',
...
export default function MessengerClient({ users, currentUserId, currentUserName }: MessengerClientProps) {
  ...
  return (
    <div className="grid h-[calc(100vh-8rem)] gap-5 lg:grid-cols-[280px_1fr]">
      {/* Contacts panel */}
      <section className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
        {/* Header */}
        <div className="border-b border-slate-100 dark:border-slate-700 px-5 py-4">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500">{contacts.length} contacts</p>
        </div>
        ...
        <ul className="flex-1 overflow-y-auto divide-y divide-slate-50 dark:divide-slate-700">
          {contacts.length === 0 ? (
            <li className="flex flex-col items-center gap-2 py-12 text-center">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">{search ? 'No contacts found.' : 'No users to chat with.'}</p>
            </li>
          ) : (
            contacts.map((user) => (
              <li
                ...
                className={cn(
                  ...
                  selected?.id === user.id ? 'bg-accent-50 dark:bg-accent-950/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50',
                )}
              >
                {user.avatarUrl ? (
                  <img
                ...
                <div className="min-w-0 flex-1">
                  <p className={cn('truncate text-sm font-semibold', selected?.id === user.id ? 'text-accent-700 dark:text-accent-400' : 'text-slate-800 dark:text-slate-100')}>
        ...
        {selected ? (
          <>
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-700 px-5 py-4">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{selected.name ?? 'Unknown'}</p>
                <p className="text-[11px] text-slate-400 dark:text-slate-500">{selected.email}</p>
              </div>
            </div>
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center p-8">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-500">
              <MessageSquare className="h-8 w-8" />
            </div>
            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">Select a contact to start chatting</p>
            <p className="text-xs text-slate-400 dark:text-slate-500">Real-time messages powered by Supabase</p>
          </div>
        )}
      </section>
    </div>
